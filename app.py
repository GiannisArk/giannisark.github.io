...
def get_all(num):
    ...
    keys = ("versionCode","version","package_name","icon","collect","share","collect_discr","share_discr","name","greek_name","downloads","type","mode")
    try:
        cursor.execute("SELECT COUNT(*) FROM Data_Accessed")
        rows_len = cursor.fetchone()
        
        num = min(num*100, int(rows_len[0]))

        qry = "SELECT * FROM Data_Accessed ORDER BY name ASC LIMIT 100 OFFSET ?"
